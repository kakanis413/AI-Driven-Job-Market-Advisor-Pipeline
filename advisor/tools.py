...
def get_dynamic_top_careers(
    ...
    """Return the pre-ranked top careers for a college major.

    Rankings are generated by the data pipeline and stored in data.json.
    Preserve that ranking order instead of recalculating it here.
    """
    major = get_major_data(major_name)

    if major.get("status") == "not_found":
        return {
            "status": "not_found",
            "major": major_name,
            "careers": [],
            "message": "The requested major was not found in the local dataset.",
        }

    requested_count = max(1, min(int(n), 10))
    raw_careers = major.get("top_careers") or []

    careers = [
        {
            "rank": index,
            "soc": career.get("soc_code"),
            "title": career.get("occupation_title"),
            "median_pay": career.get("median_wage_annual"),
            "growth": career.get("outlook_pct"),
        }
        for index, career in enumerate(raw_careers[:requested_count], start=1)
    ...
    if not careers:
        return {
            ...
            "count": 0,
            "careers": [],
            "message": "No verified top-career rankings are available for this major.",
        }

    return {
        "status": (
            "success"
            if len(careers) == requested_count
            else "partial"
        ),
        "source": "local_cache",
        "major": major.get("major", major_name),
