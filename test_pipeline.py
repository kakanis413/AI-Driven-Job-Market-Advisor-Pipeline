...
def test_process_bigquery_results():
    """Test processing of BigQuery results with new schema."""
    mock_rows = [
        MockBigQueryRow({
    ...
    assert len(result) == 2
    # Check output has correct fields (no cip, only *_norm for metrics)
    assert result[0]["major"] == "Computer Science"
    assert result[0]["family"] == "11"
    assert result[0]["graduates"] == 35000
...
def test_process_bigquery_results_output_schema():
    """Test that output contains exactly the expected fields."""
    mock_rows = [
        MockBigQueryRow({
            "major": "Agriculture",
            "family": "01",
