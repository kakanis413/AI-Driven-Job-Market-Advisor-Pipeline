...
export function Segmented<T extends string>({
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="micro hidden text-ink3 lg:block">{label}</span>
      <div
        role="group"
        aria-label={label}
        className="inline-flex items-center gap-0.5 rounded-[10px] border border-line bg-surface p-1"
      >
        {options.map((o) => {
          ...
          return (
            <button
              ...
              className={`relative h-8 rounded-lg px-3 text-[13px] font-medium transition-colors ${
